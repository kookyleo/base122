...
fn run_demo() {
    ...
    let test_cases = vec![
        ...
        ("Text with\ndangerous\rcharacters\"&\\", "Dangerous characters"),
    ];
    ...
    for (input, description) in test_cases {
        ...
        let base64_size = (input.len() * 4 + 2) / 3;
        let base122_size = encoded.as_bytes().len();
        ...
        println!("     Base122:  {} bytes ({:.1}% savings)", base122_size, savings);
        println!();
    }
    ...
    println!("Encoded length: {} bytes", encoded.as_bytes().len());
    ...
    let expansion_ratio = encoded.as_bytes().len() as f64 / binary_data.len() as f64;
...
fn run_benchmark() {
    ...
    for &size in &test_sizes {
        ...
        let expansion_ratio = encoded.as_bytes().len() as f64 / size as f64;
        let efficiency = (size as f64 / encoded.as_bytes().len() as f64) * 100.0;
        
        // Base64 comparison
        let base64_size = (size * 4 + 2) / 3;
        let vs_base64 = (base64_size as f64 - encoded.as_bytes().len() as f64) / base64_size as f64 * 100.0;
        
        println!("{:>10} {:>12} {:>12.3} {:>9.1}% {:>11.1}%",
                size, encoded.as_bytes().len(), expansion_ratio, efficiency, vs_base64);
        
        if size <= 1000 {  // Only show timing for smaller sizes
    ...
    for &density in &densities {
        ...
        let encoded = encode(&test_data);
        let efficiency = (test_data.len() as f64 / encoded.as_bytes().len() as f64) * 100.0;
        
        println!("Dangerous char density {:.0}%: efficiency {:.1}%", 
                density * 100.0, efficiency);
    }
